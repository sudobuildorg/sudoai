...
export default function AdminPage() {
  ...
  return <main className="admin-shell"><header className="admin-header"><div><div className="admin-brand">Sudo<span>AI</span></div><p>Admin Dashboard</p></div><button onClick={load}>Refresh</button></header>
    ...
    <section className="admin-card"><h2>Customers</h2><div className="admin-table-wrap"><table><thead><tr><th>User</th><th>Plan</th><th>Token balance</th><th>Messages</th><th>Activated</th></tr></thead><tbody>{users.map(u => <tr key={u.id}><td>{u.email || u.full_name || u.id}</td><td><b>{planLabels[u.plan] || u.plan || 'Free'}</b></td><td>{Number(u.token_balance || 0).toLocaleString()} / {Number(u.tokens_used || 0).toLocaleString()} used</td><td>{u.messages_used || 0} / {u.message_balance || 0}</td><td>{u.plan_activated_at ? new Date(u.plan_activated_at).toLocaleDateString() : '—'}</td></tr>)}</tbody></table></div></section>
    ...
    <style jsx>{`.admin-shell{min-height:100vh;background:#08111f;color:#eef4ff;padding:28px;font-family:system-ui,sans-serif}.admin-header{display:flex;justify-content:space-between;align-items:center;max-width:1400px;margin:0 auto 22px}.admin-brand{font-size:30px;font-weight:800}.admin-brand span{font-weight:400}.admin-header p{margin:4px 0;color:#91a2b9}.admin-header button,.admin-card button{border:1px solid #31425d;background:#142238;color:#fff;border-radius:9px;padding:9px 12px;cursor:pointer}.admin-stats{max-width:1400px;margin:0 auto 18px;display:grid;grid-template-columns:repeat(8,minmax(0,1fr));gap:10px}.admin-stat{background:#0e1a2b;border:1px solid #22344d;border-radius:14px;padding:14px}.admin-stat small{display:block;color:#8fa1b8;font-size:11px}.admin-stat strong{display:block;font-size:20px;margin-top:7px}.admin-card{max-width:1400px;margin:0 auto 18px;background:#0e1a2b;border:1px solid #22344d;border-radius:18px;padding:18px}.admin-card h2{margin:0 0 14px}.admin-help,.profitNote{color:#91a2b9;font-size:12px}.admin-table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;min-width:900px}th,td{text-align:left;padding:11px 10px;border-bottom:1px solid #1d2c42;font-size:13px}th{color:#8fa1b8;font-size:11px;text-transform:uppercase;letter-spacing:.04em}.profitGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.profitGrid div{background:#132238;border:1px solid #253a55;border-radius:12px;padding:14px}.profitGrid span{display:block;color:#8fa1b8;font-size:12px}.profitGrid b{display:block;font-size:22px;margin-top:5px}.profitPlans{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.profitPlans span{background:#142238;border:1px solid #263b57;padding:8px 10px;border-radius:999px;font-size:12px}.status{padding:4px 8px;border-radius:999px;font-size:11px}.status.pending{background:#3a2c10;color:#ffd27a}.status.approved{background:#123522;color:#8ff0b0}.status.rejected{background:#3a1820;color:#ff9daa}@media(max-width:1000px){.admin-stats{grid-template-columns:repeat(4,1fr)}}@media(max-width:600px){.admin-shell{padding:14px}.admin-stats{grid-template-columns:repeat(2,1fr)}.profitGrid{grid-template-columns:1fr}.admin-header{align-items:flex-start}}`}</style>
  </main>;
}
