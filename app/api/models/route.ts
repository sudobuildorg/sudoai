...
export async function GET(){
 ...
 const groqKey=process.env.GROQ_API_KEY; const openaiKey=process.env.OPENAI_API_KEY; const anthropicKey=process.env.ANTHROPIC_API_KEY; const geminiKey=process.env.GEMINI_API_KEY;
 if(groqKey)tasks.push((async()=>{const data=await list('https://api.groq.com/openai/v1/models',{Authorization:`Bearer ${groqKey}`,'Content-Type':'application/json'});for(const m of data){const id=String(m.id||'');if(!id||/whisper|guard|embed|moderation|audio|tts|orpheus/i.test(id))continue;models.push({id,label:id==='llama-3.1-8b-instant'?'Llama 3.1 8B Instant':id==='llama-3.3-70b-versatile'?'Llama 3.3 70B Versatile':labelFor(id),provider:'Groq'});}})());
 if(openaiKey)tasks.push((async()=>{const data=await list('https://api.openai.com/v1/models',{Authorization:`Bearer ${openaiKey}`,'Content-Type':'application/json'});for(const m of data){const id=String(m.id||'');if(!id||/embedding|whisper|tts|dall-e|moderation|search|transcribe|audio|image|realtime|sora|babbage|davinci|instruct/i.test(id))continue;models.push({id,label:id==='gpt-5.6-luna'?'GPT-5.6 Luna':id==='gpt-5.6-terra'?'GPT-5.6 Terra':id==='gpt-5.6-sol'?'GPT-5.6 Sol':labelFor(id),provider:'OpenAI'});}})());
 if(anthropicKey)tasks.push((async()=>{const data=await list('https://api.anthropic.com/v1/models',{'x-api-key':anthropicKey,'anthropic-version':'2023-06-01','Content-Type':'application/json'});for(const m of data){const id=String(m.id||'');if(!id)continue;models.push({id,label:id==='claude-sonnet-4-5'?'Claude Sonnet 4.5':id==='claude-opus-4-1'?'Claude Opus 4.1':(m.display_name||labelFor(id)),provider:'Anthropic'});}})());
 if(geminiKey)tasks.push((async()=>{try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(geminiKey)}`,{cache:'no-store'});if(!r.ok)return;const data=await r.json();for(const m of Array.isArray(data?.models)?data.models:[]){const full=String(m.name||'');const id=full.replace(/^models\//,'');const methods=Array.isArray(m.supportedGenerationMethods)?m.supportedGenerationMethods:[];if(!id||!methods.includes('generateContent')||/embedding|aqa|robotics|image-generation|tts|audio/i.test(id))continue;const free=/^gemini-2\.5-flash(?:$|-)/i.test(id)||/^gemini-2\.0-flash(?:$|-)/i.test(id);models.push({id,label:`${m.displayName||labelFor(id)}${free?' (Free)':''}`,provider:'Google'});}}catch{}})());
